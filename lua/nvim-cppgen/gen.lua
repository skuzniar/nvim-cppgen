...
local function interesting(node)
    return node.role == "declaration" and (node.kind == "CXXRecord" or node.kind == "Enum")
end
...
local function select_node(nodes)
    ...
    if nodes.enclosing and interesting(nodes.enclosing) then
    ...
    if not nodes.enclosing and nodes.preceding and interesting(nodes.preceding) then
